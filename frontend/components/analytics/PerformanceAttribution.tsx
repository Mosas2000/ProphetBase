...
export default function PerformanceAttribution() {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-5xl">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Performance Attribution</h2>
      <div className="text-center p-6 bg-green-50 rounded-lg mb-6">
        <div className="text-4xl font-bold text-green-600">${attribution.totalReturn.toLocaleString()}</div>
        ...
        {attribution.factors.map((factor, i) => (
          <div key={i} className="p-4 bg-gray-50 rounded-lg">
            <div className="flex justify-between mb-2">
              ...
              <span className="text-green-600 font-bold">${factor.amount.toLocaleString()}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div className="bg-green-600 h-3 rounded-full" style={{ width: `${factor.contribution}%` }} />
            </div>
            <div className="text-right text-sm text-gray-600 mt-1">{factor.contribution}%</div>
          </div>
        ))}
      </div>
    </div>
