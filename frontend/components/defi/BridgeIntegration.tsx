...
export default function BridgeIntegration() {
  ...
  const totalFee = amount
    ? (networks[fromNetwork].bridgeFee + networks[toNetwork].gasPrice).toFixed(
        4
      )
    : '0.00';

  const estimatedReceive = amount
    ? (parseFloat(amount) - parseFloat(totalFee)).toFixed(4)
    : '0.00';
        ...
        <div className="mb-3">
          ...
          <div className="border-2 border-gray-200 rounded-lg p-4 focus-within:border-blue-500 transition-colors">
            ...
            <div className="text-sm text-gray-500 mt-2">
              Gas: ${networks[fromNetwork].gasPrice.toFixed(4)} | Fee:{' '}
              {networks[fromNetwork].bridgeFee}%
            </div>
          </div>
        </div>
          ...
          <div className="mb-4 p-4 bg-blue-50 rounded-lg border-2 border-blue-200">
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-gray-900">
                  ${networks[fromNetwork].gasPrice.toFixed(4)}
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-3">
                  <span className="text-xl">
                    {networks[tx.from.toLowerCase()].logo}
                  </span>
                  <span className="text-gray-700">→</span>
                  <span className="text-xl">
                    {networks[tx.to.toLowerCase()].logo}
                  </span>
                  <div>
      ...
      <div className="mt-6 p-6 bg-gray-50 rounded-lg">
        ...
        <div className="grid grid-cols-5 gap-3">
          {Object.values(networks).map((network) => (
            <div
              key={network.id}
              className="text-center p-3 bg-white rounded-lg border-2 border-gray-200"
            >
