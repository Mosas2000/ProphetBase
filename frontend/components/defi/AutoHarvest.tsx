...
import { Clock, Zap, TrendingUp, Settings, Play, Pause, CheckCircle, AlertTriangle } from 'lucide-react';
...
export default function AutoHarvest() {
  ...
  const [nextHarvest, setNextHarvest] = useState<Date>(new Date(Date.now() + 1000 * 60 * 60 * 2));
  const [currentGasPrice, setCurrentGasPrice] = useState(25);

  useEffect(() => {
    // Simulate gas price updates
    const interval = setInterval(() => {
      setCurrentGasPrice(prev => Math.max(10, Math.min(100, prev + (Math.random() - 0.5) * 10)));
    }, 5000);
    return () => clearInterval(interval);
  }, []);

  const totalPendingRewards = sources.reduce((sum, source) => sum + source.pendingRewards, 0);
  const totalEstimatedGas = sources.reduce((sum, source) => sum + source.estimatedGas, 0);
  ...
  const canHarvest = (source: HarvestSource) => {
    return source.pendingRewards >= config.minRewardThreshold && currentGasPrice <= config.maxGasPrice;
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-slate-400">Automated reward collection and compounding</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700">
            <div className="flex items-center justify-between mb-2">
              ...
              <AlertTriangle className={`w-4 h-4 ${currentGasPrice > config.maxGasPrice ? 'text-red-400' : 'text-green-400'}`} />
            </div>
            <div className="text-2xl font-bold">
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div>
              <label className="block text-sm text-slate-400 mb-2">Min Reward Threshold</label>
              <input
                type="number"
                value={config.minRewardThreshold}
                onChange={(e) => setConfig(prev => ({ ...prev, minRewardThreshold: Number(e.target.value) }))}
            ...
            <div>
              <label className="block text-sm text-slate-400 mb-2">Max Gas Price (Gwei)</label>
              <input
                type="number"
                value={config.maxGasPrice}
                onChange={(e) => setConfig(prev => ({ ...prev, maxGasPrice: Number(e.target.value) }))}
                className="w-full px-4 py-3 bg-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
            ...
            <div>
              <label className="block text-sm text-slate-400 mb-2">Auto Compound</label>
              <button
                onClick={() => setConfig(prev => ({ ...prev, compound: !prev.compound }))}
                className={`w-full px-4 py-3 rounded-lg font-medium transition-colors ${
                  config.compound
            ...
            <div>
              <label className="block text-sm text-slate-400 mb-2">Gas Optimization</label>
              <button
                onClick={() => setConfig(prev => ({ ...prev, gasOptimization: !prev.gasOptimization }))}
                className={`w-full px-4 py-3 rounded-lg font-medium transition-colors ${
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <div className="text-xs text-slate-400">{source.rewardToken}</div>
                    </div>
                    ...
                    >
                      {source.autoHarvest ? <Play className="w-5 h-5" /> : <Pause className="w-5 h-5" />}
                    </button>

                    <button
                      onClick={() => handleHarvestSingle(source.id)}
                      disabled={isHarvesting || source.pendingRewards === 0 || !canHarvest(source)}
