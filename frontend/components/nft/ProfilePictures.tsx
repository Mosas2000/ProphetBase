...
import { Camera, Check, Download, Palette, RefreshCw, Save, Shuffle, Sparkles, Upload, User } from 'lucide-react';
...
export default function ProfilePictures() {
  ...
  const [selectedTraitCategory, setSelectedTraitCategory] = useState<string>('background');
  
  const generatorOptions: GeneratorOptions = {
    background: [
      { category: 'background', name: 'Cosmic Purple', rarity: 'common', icon: '🌌', color: '#6B21A8' },
      { category: 'background', name: 'Ocean Blue', rarity: 'common', icon: '🌊', color: '#1E40AF' },
      { category: 'background', name: 'Forest Green', rarity: 'uncommon', icon: '🌲', color: '#065F46' },
      { category: 'background', name: 'Golden Sunset', rarity: 'rare', icon: '🌅', color: '#D97706' },
      { category: 'background', name: 'Diamond Matrix', rarity: 'legendary', icon: '💎', color: '#0EA5E9' }
    ],
    body: [
      { category: 'body', name: 'Classic Avatar', rarity: 'common', icon: '👤', color: '#E5E7EB' },
      { category: 'body', name: 'Bull Trader', rarity: 'uncommon', icon: '🐂', color: '#10B981' },
      { category: 'body', name: 'Bear Analyst', rarity: 'uncommon', icon: '🐻', color: '#EF4444' },
      { category: 'body', name: 'Crypto Punk', rarity: 'rare', icon: '🤖', color: '#8B5CF6' },
      { category: 'body', name: 'Legendary Ape', rarity: 'legendary', icon: '🦧', color: '#F59E0B' }
    ],
    eyes: [
      { category: 'eyes', name: 'Normal Eyes', rarity: 'common', icon: '👁️', color: '#FFFFFF' },
      { category: 'eyes', name: 'Laser Eyes', rarity: 'uncommon', icon: '👀', color: '#EF4444' },
      { category: 'eyes', name: 'Diamond Eyes', rarity: 'rare', icon: '💎', color: '#3B82F6' },
      { category: 'eyes', name: 'Rainbow Vision', rarity: 'legendary', icon: '🌈', color: '#EC4899' }
    ],
    mouth: [
      { category: 'mouth', name: 'Smile', rarity: 'common', icon: '😊', color: '#FFFFFF' },
      { category: 'mouth', name: 'Cool Shades', rarity: 'uncommon', icon: '😎', color: '#000000' },
      { category: 'mouth', name: 'Money Mouth', rarity: 'rare', icon: '🤑', color: '#10B981' },
      { category: 'mouth', name: 'Golden Grill', rarity: 'legendary', icon: '👄', color: '#F59E0B' }
    ],
    accessory: [
      { category: 'accessory', name: 'None', rarity: 'common', icon: '⚪', color: '#6B7280' },
      { category: 'accessory', name: 'Trader Hat', rarity: 'uncommon', icon: '🎩', color: '#000000' },
      { category: 'accessory', name: 'Crown', rarity: 'rare', icon: '👑', color: '#F59E0B' },
      { category: 'accessory', name: 'Halo', rarity: 'legendary', icon: '😇', color: '#FBBF24' }
    ],
    special: [
      { category: 'special', name: 'None', rarity: 'common', icon: '', color: '#6B7280' },
      { category: 'special', name: 'Sparkles', rarity: 'uncommon', icon: '✨', color: '#FBBF24' },
      { category: 'special', name: 'Fire Aura', rarity: 'rare', icon: '🔥', color: '#EF4444' },
      { category: 'special', name: 'Lightning', rarity: 'legendary', icon: '⚡', color: '#FBBF24' }
    ]
  };
  ...
  const generateRandom = () => {
    ...
    setTimeout(() => {
      setCustomProfile({
        background: generatorOptions.background[Math.floor(Math.random() * generatorOptions.background.length)],
        body: generatorOptions.body[Math.floor(Math.random() * generatorOptions.body.length)],
        eyes: generatorOptions.eyes[Math.floor(Math.random() * generatorOptions.eyes.length)],
        mouth: generatorOptions.mouth[Math.floor(Math.random() * generatorOptions.mouth.length)],
        accessory: generatorOptions.accessory[Math.floor(Math.random() * generatorOptions.accessory.length)],
        special: Math.random() > 0.7 ? generatorOptions.special[Math.floor(Math.random() * generatorOptions.special.length)] : null
      });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-bold">NFT Profile Pictures</h1>
              <p className="text-slate-400">Create unique avatars with trait-based generative art</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          >
            ...
            <p className="text-sm text-slate-400">Create your unique profile NFT</p>
          </button>
          ...
          >
            ...
            <p className="text-sm text-slate-400">{savedProfiles.length} profiles saved</p>
          </button>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700">
            ...
            <p className="text-sm text-slate-400">
              {savedProfiles.find(p => p.equipped)?.name || 'No profile equipped'}
            </p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl p-8 border border-slate-700">
                  ...
                  >
                    <div className="text-8xl mb-4">{customProfile.body.icon}</div>
                  ...
                  <div className="mt-4 text-center">
                    <div className="text-sm text-slate-400 mb-1">Rarity Score</div>
                    <div className="text-2xl font-bold">{calculateRarity(customProfile).toFixed(1)}%</div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="mb-4">
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {['background', 'body', 'eyes', 'mouth', 'accessory', 'special'].map((category) => (
                      <button
                        key={category}
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {generatorOptions[selectedTraitCategory as keyof GeneratorOptions]?.map((trait: Trait) => (
                    <button
                      key={trait.name}
                      onClick={() => selectTrait(trait)}
                      className={`w-full p-4 rounded-lg border transition-all ${
                        customProfile[selectedTraitCategory as keyof typeof customProfile]?.name === trait.name
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-left">
                            ...
                            <div className={`text-xs px-2 py-1 rounded inline-block ${getRarityColor(trait.rarity)}`}>
                              {trait.rarity}
                            </div>
                          </div>
                        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {savedProfiles.map((profile) => (
                ...
                >
                  ...
                  >
                    <div className="text-6xl mb-3">{profile.traits.body.icon}</div>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <div className="font-semibold">{profile.rarity.toFixed(1)}%</div>
                      </div>
                      {profile.tokenId && (
                        <div className="text-right">
                          <div className="text-xs text-slate-400">Token ID</div>
                          <div className="text-xs font-mono">{profile.tokenId.slice(0, 8)}...</div>
                        </div>
                      )}
                    </div>
        ...
        <div className="bg-blue-600/10 border border-blue-600/20 rounded-xl p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-400 mb-2">About Profile NFTs</h3>
              <p className="text-sm text-slate-300">
                Create unique profile pictures using our trait-based generative system. Each combination has a rarity score based on trait rarity. 
                Mint your favorite designs as NFTs and use them as your profile picture across the platform. Some traits are exclusive to high-performing traders!
              </p>
            </div>
          </div>
        </div>
