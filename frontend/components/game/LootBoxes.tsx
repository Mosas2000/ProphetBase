...
import { Package, Sparkles, Gift, Lock, TrendingUp } from 'lucide-react';
...
export default function LootBoxes() {
  ...
  const [boxes] = useState<LootBox[]>([
    ...
    { id: 'b4', name: 'Diamond Box', rarity: 'legendary', cost: 1000, owned: 0 }
  ]);

  const lootPool: LootItem[] = [
    { name: '50 Tokens', type: 'token', rarity: 'common', value: 50, icon: '💰' },
    { name: '100 Tokens', type: 'token', rarity: 'common', value: 100, icon: '💰' },
    { name: '250 Tokens', type: 'token', rarity: 'rare', value: 250, icon: '💎' },
    { name: '500 Tokens', type: 'token', rarity: 'epic', value: 500, icon: '💎' },
    { name: '1000 Tokens', type: 'token', rarity: 'legendary', value: 1000, icon: '👑' },
    { name: '2x XP Boost', type: 'boost', rarity: 'rare', value: 1, icon: '🚀' },
    { name: '3x XP Boost', type: 'boost', rarity: 'epic', value: 1, icon: '🔥' },
    { name: 'Rare NFT', type: 'nft', rarity: 'rare', value: 300, icon: '🎨' },
    { name: 'Epic NFT', type: 'nft', rarity: 'epic', value: 800, icon: '🖼️' },
    { name: 'Legendary NFT', type: 'nft', rarity: 'legendary', value: 2000, icon: '👑' },
    { name: 'Rare Avatar', type: 'cosmetic', rarity: 'rare', value: 200, icon: '😎' },
    { name: 'Epic Avatar', type: 'cosmetic', rarity: 'epic', value: 500, icon: '🤩' }
  ];
  ...
  const openBox = (box: LootBox) => {
    ...
    setTimeout(() => {
      const rarityWeights: Record<string, number> = {
        common: box.rarity === 'common' ? 60 : box.rarity === 'rare' ? 40 : box.rarity === 'epic' ? 20 : 10,
        rare: box.rarity === 'common' ? 30 : box.rarity === 'rare' ? 40 : box.rarity === 'epic' ? 30 : 20,
        epic: box.rarity === 'common' ? 9 : box.rarity === 'rare' ? 18 : box.rarity === 'epic' ? 40 : 30,
        legendary: box.rarity === 'common' ? 1 : box.rarity === 'rare' ? 2 : box.rarity === 'epic' ? 10 : 40
      };
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-slate-400">Open mystery boxes for rewards and rare items</p>
          </div>
        </div>
          ...
          {boxes.map(box => (
            ...
            >
              <div className="text-center mb-4">
                ...
                <h3 className={`text-xl font-bold mb-2 ${getRarityColor(box.rarity)}`}>
                  {box.name}
                </h3>
                <div className={`inline-block px-3 py-1 rounded-full text-xs font-bold mb-3 ${getRarityBg(box.rarity)}`}>
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700">
          <h2 className="text-2xl font-bold mb-4">Drop Rates & Probabilities</h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {boxes.map(box => (
              <div key={box.id}>
                <h3 className={`font-bold mb-3 ${getRarityColor(box.rarity)}`}>{box.name}</h3>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-slate-400">Common:</span>
                    <span>{box.rarity === 'common' ? '60%' : box.rarity === 'rare' ? '40%' : box.rarity === 'epic' ? '20%' : '10%'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-blue-400">Rare:</span>
                    <span>{box.rarity === 'common' ? '30%' : box.rarity === 'rare' ? '40%' : box.rarity === 'epic' ? '30%' : '20%'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-purple-400">Epic:</span>
                    <span>{box.rarity === 'common' ? '9%' : box.rarity === 'rare' ? '18%' : box.rarity === 'epic' ? '40%' : '30%'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-amber-400">Legendary:</span>
                    <span>{box.rarity === 'common' ? '1%' : box.rarity === 'rare' ? '2%' : box.rarity === 'epic' ? '10%' : '40%'}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        {revealedItem && !opening && (
          ...
          >
            <div
              className={`rounded-xl p-8 border-2 max-w-md w-full text-center ${getRarityBg(revealedItem.rarity)}`}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="text-8xl mb-4 animate-bounce">{revealedItem.icon}</div>
              <div className={`inline-block px-4 py-2 rounded-full text-sm font-bold mb-4 ${getRarityBg(revealedItem.rarity)}`}>
                {revealedItem.rarity.toUpperCase()}
              </div>
              <h2 className={`text-3xl font-bold mb-2 ${getRarityColor(revealedItem.rarity)}`}>
                {revealedItem.name}
              </h2>
              <p className="text-slate-300 mb-6">You got {revealedItem.name}!</p>
              <button
                onClick={() => setRevealedItem(null)}
