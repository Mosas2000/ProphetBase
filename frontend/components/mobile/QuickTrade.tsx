...
export const QuickTrade: React.FC<QuickTradeProps> = ({ onTrade, symbol, price }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white shadow-lg p-4 flex flex-col gap-2 z-50 md:hidden">
      <div className="flex justify-between items-center">
        <span className="font-bold text-lg">{symbol} @ {price}</span>
        <button
      ...
      <div className="flex gap-2 items-center">
        <button
          className={`px-4 py-2 rounded ${side === 'buy' ? 'bg-green-500 text-white' : 'bg-gray-100'}`}
          onClick={() => setSide('buy')}
        >Buy</button>
        <button
          className={`px-4 py-2 rounded ${side === 'sell' ? 'bg-red-500 text-white' : 'bg-gray-100'}`}
