...
export default function PerformanceMonitor() {
  const [metrics, setMetrics] = useState<PerformanceMetric[]>([
    { name: 'Response Time', value: 247, unit: 'ms', status: 'good', trend: 'stable' },
    { name: 'Requests/sec', value: 142, unit: 'req/s', status: 'good', trend: 'up' },
    ...
    { name: 'DB Query Time', value: 89, unit: 'ms', status: 'good', trend: 'stable' }
  ]);

  const [history, setHistory] = useState<number[][]>(
    metrics.map(() => Array(20).fill(0).map(() => Math.random() * 100))
  );
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-7xl">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {metrics.map((metric, index) => (
          <div key={metric.name} className={`border-2 rounded-lg p-5 ${getStatusColor(metric.status)}`}>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="border-2 border-gray-200 rounded-lg p-5">
          <h3 className="font-semibold text-gray-900 mb-4">Slowest Endpoints</h3>
                  ...
                  <div className="w-32 bg-gray-200 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${
                        item.time > 500 ? 'bg-red-500' :
                        item.time > 300 ? 'bg-yellow-500' : 'bg-green-500'
                      }`}
                      style={{ width: `${Math.min((item.time / 1000) * 100, 100)}%` }}
                    />
                  </div>
      ...
      <div className="mt-6 p-6 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg border-2 border-blue-200">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-gray-900 mb-1">System Health Score</h3>
      ...
      <div className="mt-6">
        ...
        <div className="space-y-2">
          {[
            { time: '2 mins ago', message: 'Memory usage exceeded 60%', severity: 'warning' },
            { time: '15 mins ago', message: 'Slow query detected on trades table', severity: 'warning' },
            { time: '1 hour ago', message: 'API rate limit reached for user 0x1234...', severity: 'info' }
          ].map((alert, i) => (
