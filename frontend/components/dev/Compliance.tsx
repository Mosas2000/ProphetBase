...
export default function Compliance() {
  ...
  const rules: ComplianceRule[] = [
    {
      ...
      description: 'Users must complete identity verification before trading above $1000',
      category: 'kyc',
      status: 'passing',
    ...
    },
    {
      ...
      description: 'Monitor transactions for suspicious patterns and amounts above $10,000',
      category: 'aml',
      status: 'passing',
    ...
    },
    {
      ...
      description: 'User data must be encrypted and deletable within 30 days upon request',
      category: 'data',
      status: 'passing',
    ...
    },
    {
      ...
      description: 'Personally Identifiable Information must be properly masked in logs',
      category: 'data',
      status: 'passing',
      lastCheck: new Date(Date.now() - 3600000)
    },
    {
      ...
      description: 'Screen users against OFAC and international sanctions lists',
      category: 'aml',
      status: 'failing',
    ...
    },
    {
      id: '8',
      name: 'Audit Logging',
      description: 'All financial transactions must be logged and retained for 7 years',
      category: 'security',
      status: 'passing',
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      ...
      case 'aml': return 'bg-purple-100 text-purple-700';
  ...
  const complianceScore = Math.round(((stats.passing + stats.warning * 0.5) / stats.total) * 100);
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-7xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600">Monitor regulatory compliance and security standards</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="p-4 bg-green-50 rounded-lg border-2 border-green-200">
          <div className="text-3xl font-bold text-green-700">{stats.passing}</div>
              ...
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <span className={`text-xs px-2 py-1 rounded font-semibold uppercase ${getCategoryColor(rule.category)}`}>
                      {rule.category}
                    </span>
                  </div>
                </div>
        ...
        <div>
          {selectedRule ? (
            <div className="border-2 border-gray-200 rounded-lg p-6 sticky top-0">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-gray-900">{selectedRule.name}</h3>
                <button
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="mt-1 flex items-center space-x-2">
                    <span className="text-3xl">{getStatusIcon(selectedRule.status)}</span>
                    <span className={`text-xl font-bold capitalize ${
                      selectedRule.status === 'passing' ? 'text-green-600' :
                      selectedRule.status === 'failing' ? 'text-red-600' :
                      'text-yellow-600'
                    }`}>
                      {selectedRule.status}
                    </span>
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <span className={`px-3 py-1 rounded font-semibold uppercase ${getCategoryColor(selectedRule.category)}`}>
                ...
                <div>
                  <label className="text-xs font-semibold text-gray-600 uppercase">Last Check</label>
                  <p className="mt-1 text-gray-900">{selectedRule.lastCheck.toLocaleString()}</p>
                </div>

                {selectedRule.status === 'failing' && (
                  <div className="p-4 bg-red-50 rounded-lg border-2 border-red-200">
                    <h4 className="font-semibold text-red-700 mb-2">⚠️ Action Required</h4>
                    <p className="text-sm text-red-600 mb-3">
                      This rule is currently failing and requires immediate attention to maintain compliance.
                    </p>
                ...
                {selectedRule.status === 'warning' && (
                  <div className="p-4 bg-yellow-50 rounded-lg border-2 border-yellow-200">
                    <h4 className="font-semibold text-yellow-700 mb-2">⚠️ Warning</h4>
                    <p className="text-sm text-yellow-600">
                      This rule requires attention soon to prevent compliance issues.
                    </p>
                  </div>
                )}
      ...
      <div className="mt-6 p-6 bg-gray-50 rounded-lg">
        <h3 className="font-semibold text-gray-900 mb-4">📋 Applicable Regulations</h3>
