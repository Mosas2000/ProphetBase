    ...
    return (
        ...
        >
            ...
            <div className="mb-4 flex items-start justify-between gap-3">
                ...
                <span className="text-xs text-gray-500 font-mono">#{position.marketId}</span>
            </div>
            ...
            <div className="mb-4 space-y-2">
                {position.yesShares > BigInt(0) && (
                    <div className="flex items-center justify-between rounded-lg bg-green-50 border border-green-200 p-3">
                        <span className="text-sm font-medium text-green-700">YES Shares</span>
            ...
            {/* Resolved Outcome */}
            {isResolved && (
                <div className={`mb-4 rounded-lg p-3 text-center ${position.market.outcome ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'
                    }`}>
                    <div className="text-xs font-medium text-gray-600 mb-1">Market Outcome</div>
                    <div className={`text-lg font-bold ${position.market.outcome ? 'text-green-700' : 'text-red-700'
                        }`}>
    ...
    const positions: UserPosition[] = marketIds
        .map((id) => {
            ...
            const isWinning =
                ...
                ((market.outcome && yesShares > BigInt(0)) || (!market.outcome && noShares > BigInt(0)))
    ...
    if (positions.length === 0) {
        return (
            <div className="rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 p-12 text-center">
                ...
                <h3 className="mb-2 text-lg font-semibold text-gray-900">No Positions Yet</h3>
                <p className="text-gray-600">You haven't purchased any shares yet. Start trading to see your positions here!</p>
            </div>
        )
    }
